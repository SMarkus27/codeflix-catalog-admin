...
class CreateVideoWithoutMedia:
    ...
    def execute(self, input: Input) -> Output:
        ...
        self.validate_cast_member(input, notification)
    ...
    def validate_categories(self, input: Input, notification: Notification):
        categories = {category.id for category in self.category_repository.list()}

        if not input.categories.issubset(categories):
            notification.add_error(f"Categories with provided IDs not found")

    def validate_cast_member(self, input: Input, notification: Notification):
        ...
        if not input.cast_members.issubset(cast_member_ids):
            notification.add_error(f"Cast members with provided IDs not found")


    def validate_genres(self, input: Input, notification: Notification):
